...
var viev = {
	...
	displayNumberofHits: function () {
		...
		document.getElementById("numberOfHitArea").innerHTML = "Shots: "  +numberHits + " times!" ;
	}
};
		...
		for (var i = 0; i < this.numShips; i++) {
			...
			for (var x = 0; x < this.shipLength; x++) {
				if (target == ship.locations[x]) { //sprawdzenie czy trafilismy
					...
					if(this.isDown(ship)) {
						viev.displayMessage("You have sank my ship!");
						this.shipDown++;
					};
					return true;
				}
		...
		if (this.checkingTheSameTarget(target)) {
			...
			if (model.shipDown === 3) {
				if (this.playerHits < 20) {
					alert("YOU HAVE SINK ALL SHIPS!" + "\nYOU ARE A PRO!!!");
					return location.reload();
				} else if ((this.playerHits >= 20) && (this.playerHits < 40)) {
					alert("YOU HAVE SINK ALL SHIPS!" + "\nGood, but you can better!");
					return location.reload();
				} else {
					alert("YOU HAVE SINK ALL SHIPS!" + "\nPoor, train more!");
					return location.reload();
